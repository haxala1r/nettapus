...
uint16_t fat16_get_FAT_entry(file_system_t *fs, uint32_t entry) {
...
uint8_t fat16_load_bpb(file_system_t *fs) {
...
FAT16_DIR_ENTRY *fat16_search_directory(file_system_t *fs, char *file_name, uint32_t cluster) {
...
FAT16_FILE *fat16_file_lookup(file_system_t *fs, char *full_path) {
...
uint8_t fat16_read_file(file_system_t *fs, FAT16_FILE *file, void* buf, uint32_t offset, uint32_t bytes) {
...
uint8_t fat16_write_file(file_system_t *fs, FAT16_FILE *file, void* buf, uint32_t offset, uint32_t bytes) {
